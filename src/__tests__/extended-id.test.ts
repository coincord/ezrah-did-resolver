...
describe('interpretIdentifier', () => {
  ...
  it('parses did:ezrah with address', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toBeUndefined()
  })
  it('parses did:ezrah with address and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:${checksumAddress}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toBeUndefined()
  })
  it('parses did:ezrah with address and network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:0x1:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('0x1')
  })
  it('parses did:ezrah with address and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:rsk:testnet:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('rsk:testnet')
  })
  it('parses did:ezrah with address and sub-network and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:rsk:testnet:${checksumAddress}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('rsk:testnet')
  })
  ...
  it('parses did:ezrah with publicKey', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toBeUndefined()
  })
  it('parses did:ezrah with publicKey and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:${pubKey}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toBeUndefined()
  })
  it('parses did:ezrah with publicKey and network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:mainnet:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('mainnet')
  })
  it('parses did:ezrah with publicKey and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:not:so:main:net:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('not:so:main:net')
  })
  it('parses did:ezrah with publicKey and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:ezrah:not:so:main:net:${pubKey}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('not:so:main:net')
  })
